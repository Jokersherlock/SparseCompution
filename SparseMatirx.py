...
class SparseMatrix:
    ...
    def _generate_sparse_matrix(self):
        ...
        non_zero_elements = int(total_elements * self.sparsity)
        
        # 创建一个全为零的矩阵
        matrix = np.zeros((self.rows, self.cols), dtype=int)
        
        # 随机选择 non_zero_elements 个位置并赋值为随机数（范围 0-5）
        indices = np.random.choice(total_elements, non_zero_elements, replace=False)
        matrix.flat[indices] = np.random.randint(self.value_range[0], self.value_range[1] + 1, non_zero_elements)
